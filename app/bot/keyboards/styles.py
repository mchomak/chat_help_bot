...
# ── Russian style prompts ──────────────────────────────────────────────────────

STYLE_PROMPTS_RU: dict[str, str] = {
    "flirt": (
        "СТИЛЬ: Флиртующий и игривый. Используй тонкие намёки, двойные смыслы, лёгкое поддразнивание "
        "и создавай романтическое напряжение. Будь обаятельным, но не пошлым. "
        "Добавь интригу — заставь хотеть большего. Думай «уверенный флирт», а не «дешёвый подкат»."
    ),
    "bold": (
        "СТИЛЬ: Дерзкий, смелый, провокационный. Будь уверенно прямым — говори то, что другие не решаются. "
        "Используй острое остроумие, неожиданные суждения и смелый юмор. "
        "Думай «человек, который притягивает к себе внимание» — самоуверенно, но без грубости."
    ),
    "romantic": (
        "СТИЛЬ: Романтичный и поэтичный. Используй тёплые, искренние слова с красивыми формулировками. "
        "Создавай ощущение близости и эмоциональной глубины. Ссылайся на чувства, ощущения, значимые моменты. "
        "Думай «рукописное письмо», а не «открытка». Будь настоящим, а не слащавым."
    ),
    "intellectual": (
        "СТИЛЬ: Интеллектуальный и глубокий. Показывай искреннее любопытство и вдумчивость. "
        "Используй игру слов, отсылки и наблюдения, раскрывающие глубину. "
        "Задавай вопросы, заставляющие думать. Суть важнее поверхности — но тепло и интересно, не холодно."
    ),
    "funny": (
        "СТИЛЬ: Максимальный юмор. Будь по-настоящему смешным — остроумные наблюдения, удачные шутки, "
        "игра слов, абсурдные сравнения, самоирония. Думай «самый смешной человек в групповом чате». "
        "Каждый вариант должен вызывать смех или хотя бы улыбку. Юмор умный, а не грубый."
    ),
    "calm": (
        "СТИЛЬ: Спокойный и уверенный. Расслабленный, естественный, непринуждённый — как тот, "
        "кому комфортно в своей шкуре. Никакого «слишком стараться», никакого лишнего обдумывания. "
        "Простой, прямой язык. Интерес без погони. Меньше — значит больше."
    ),
}

DEFAULT_STYLE_DESCRIPTION_RU = "вежливо, уверенно, естественно"
...
def get_style_prompt(style_key: str | None) -> str:
    """Return AI prompt fragment for the given style (English)."""
    if style_key is None:
        return f"Tone: {DEFAULT_STYLE_DESCRIPTION}"
    return STYLE_PROMPTS.get(style_key, f"Tone: {DEFAULT_STYLE_DESCRIPTION}")


def get_style_prompt_ru(style_key: str | None) -> str:
    """Return AI prompt fragment for the given style (Russian)."""
    if style_key is None:
        return f"Тон: {DEFAULT_STYLE_DESCRIPTION_RU}"
    return STYLE_PROMPTS_RU.get(style_key, f"Тон: {DEFAULT_STYLE_DESCRIPTION_RU}")
